...
clippy:
    cargo clippy -p schemadiff -p mcplint -p mcpclient -p mcpstore -p mockserver -p probe -p webprobe -p mcprouter -p mcpi-cli --all-targets -- -D warnings
...
test:
    ...
    cargo test -p schemadiff -p mcplint -p mcpclient -p mcpstore -p probe -p webprobe -p mcprouter -p mcpi -p mcpi-cli
